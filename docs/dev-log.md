...
- 參考圖片風格，將名片改為黑色底 + 金色幾何線條花紋的設計感方向。
- 更新 `src/css/style.css`：
  - 名片本體改為 `#141414` 黑色。
  - 頁面背景改為淺暖灰 `#f0f0f0`，襯托黑色名片。
  - 職稱、聯絡標籤、分隔線、服務標籤邊框改為金銅色 `#bfa37c`。
  - 加入純 CSS 60° / −60° / 0° 重複線條，組成金色三角幾何網格，並以 `mask-image` 從左側淡入，避免干擾文字。
  - 保留 SVG feTurbulence 霧面顆粒，讓黑色不會顯得塑膠感。
  - 桌機維持橫式雙欄，手機版改為直式堆疊。
- 重寫 `docs/design.md`，將色盤、排版、元件說明同步為黑金幾何版。

### 設計決策

- 選擇品牌定位為 `brand`，因為這張名片頁面的存在本身就是為了傳達個人品牌印象，而不是操作工具或後台。
- 品牌個性確認為「安靜、溫暖、沉穩」。
- 配色以金銅色 `#bfa37c` 為主強調色，搭配近黑色名片與米白文字，呈現沉穩的設計感，同時保留霧面顆粒維持紙張質感。
